...
export default function WithdrawPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.account_id || !formData.token_address || !formData.amount || !formData.to_address) {
    ...
    try {
      ...
      if (data.success) {
        setResult(data.data);
        setFormData({ account_id: '', token_address: '', amount: '', to_address: '' });
      } else {
        setError(data.error || 'Failed to execute withdrawal');
      }
    } catch (err) {
  ...
  return (
    <Layout>
      <div className="max-w-4xl">
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-white mb-2">Withdraw</h1>
          ...
          <div>
            ...
            {result && (
              <div className="rounded-lg border border-emerald-700 bg-emerald-900/20 p-6">
                <h3 className="text-lg font-semibold text-emerald-400 mb-4">Withdrawal Successful! ✓</h3>
