...
vi.mock("@/services/withdraw", () => ({
	getWithdrawQuote: mocks.getWithdrawQuote,
	executeWithdrawQuote: mocks.executeWithdrawQuote,
}));
...
describe("withdraw command interactive", () => {
	beforeEach(() => {
		vi.clearAllMocks();
		...
		mocks.getWithdrawQuote.mockResolvedValue({
			status: "success",
			amountFormatted: "1",
			transferFeeFormatted: "0.01",
			receivedAmountFormatted: "0.99",
			quote: {},
		});
	});

	it("keeps old error when non-interactive and required flags are missing", async () => {
		const { withdrawCommand } = await import("@/commands/withdraw");
	...
	it("uses prompt helpers in interactive mode", async () => {
		...
		mocks.resolveAddressWithOptionalPrompt.mockResolvedValue("0x123");
